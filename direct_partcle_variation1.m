function  gbest_end = direct_partcle_variation1(num1,num2,num3)
    ...
    xh_max=100*ones(1,D);  %%上边界向量
    xh_min=-100*ones(1,D);  %%下边界向量
    %初始化个体
...
%%
%%试变异策略
        x_vrt=ones(1,D); %%试变异矩阵初始
      if num3==2 
         if(func1(x(j,:))<gbest)%%如果当前值优于群体最优值，替换群体最优和个体最优
            g=x(j,:);    
            gbest=func1(x(j,:));
            p(j,:)=x(j,:);
            pbest(j)=func1(x(j,:));
         elseif (func1(x(j,:))>=gbest && func1(x(j,:))<pbest(j))%%如果当前值只是优于个体最优值
            p(j,:)=x(j,:);
            pbest(j)=func1(x(j,:));
         elseif(func1(x(j,:))>=pbest(j))%%如果当前值比个体最优还差，则执行试变异策略
             x_vrt(1,:)=(rand^2)*g+(rand^2)*xh_max+(rand^2)*xh_min;  %%变异
              if(func1(x_vrt(1,:))<gbest)
                g=x_vrt(1,:);    
                gbest=func1(x_vrt(1,:));
                p(j,:)=x_vrt(1,:);
                pbest(j)=gbest;
                x(j,:)=x_vrt(1,:);  %%变异1成功
              elseif (func1(x_vrt(1,:))>=gbest && func1(x_vrt(1,:))<pbest(j))
                p(j,:)=x_vrt(1,:);
                pbest(j)=func1(x_vrt(1,:));
                x(j,:)=x_vrt(1,:);  %%变异2成功
              else
              end  
         else
